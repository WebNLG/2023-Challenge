...
class Benchmark:
    ...
    def to_dict(self):
        """Convert benchmark to Dictionary object."""
        data = {}
        data['entries'] = []
...
def select_files(path, category='', size=(1, 8)):
    if os.path.isdir(path):
        finaldirs = [path + '/' + str(item) + 'triples' for item in range(size[0], size[1])]
        finalfiles = []
        ...
        finalfiles = [(os.path.dirname(path), os.path.basename(path))]
